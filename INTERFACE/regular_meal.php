...
function fetchMenuItems($db_connection) {
    $sql = "SELECT item_id, item_name, price, image, stock FROM menu1";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    foreach ($_POST["quantity"] as $item_id => $quantity) {
        ...
        $sql_select = "SELECT price, stock FROM menu1 WHERE item_id = ?";
        ...
        $sql_update_stock = "UPDATE menu1 SET stock = ? WHERE item_id = ?";
    ...
    // Redirect back to the regular meal page after successful submission
    header("Location: regular_meal.php");
    exit();
}
?>
